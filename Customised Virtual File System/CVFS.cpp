...
void man(char *name)
{
    ...
    {
        printf("Description : Used to read data from regualr files\n");
        printf("Usage : read File_name No_Of_Bytes_To_Read\n");
    }
    else if(strcmp(name,"write") == 0)
    {
        printf("Description : Used to write data from regualr files\n");
...
int WriteFile(int fd, char *arr, int isize)
{
    ...
    (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) = UFDTArr[fd].ptrfiletable->readoffset + isize;
...
int main()
{
    ...
    while(1)    
    {
        ...
        count = sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);  //tokenation
        ...
        if(count == 1)       
        {
            ...
            {
                ls_file();  // our own logic / function
        ...
        // jya command madhe 2 words aahet
        else if(count == 2)
        {
            // file statistics 
            ...
            else if(strcmp(command[0],"write") == 0)
            {
                ...
                if(fd == -1)
                {
                    printf("Error : Incorrecg Paramaters\n");
                    continue;
                }
                ...
                if(ret == -2)
                    printf("ERROR : There is no sufficient memory to written\n");
                if(ret == -3)
